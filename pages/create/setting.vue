  ...
  <view class="create-page" :class="{ 'light-theme': !isDarkMode }">
    ...
    <view class="header">
      <view class="header-titles">
        <text class="title">新增设定\n</text>
        <text class="subtitle">只记录关键字段，其他细节在设定页统一配置</text>
      </view>
    ...
    <view class="content-card simple-card">
      <view class="content-header">
        <text class="content-title">设定信息</text>
        <text class="content-hint">名称必填，描述可选</text>
      </view>
...
const settingSeedId = ref(`setting_${Date.now()}`);
...
const settingPayload = computed(() => ({
  id: settingSeedId.value,
  type: "setting",
...
const canSubmit = computed(() => {
  return !!selectedWorkId.value && settingForm.name.trim().length > 0;
});
...
const handleSubmit = () => {
  ...
  isSaving.value = true;
  persistSetting(settingPayload.value)
    .then(() => {
      uni.showToast({
        title: "已保存",
        icon: "success",
...
const persistSetting = async (data) => {
  ...
  const settingDir = `${workPath}/settings`;
  const settingFile = `${settingDir}/custom_settings.json`;

  fileStorage.mkdirIfNotExists(settingDir);

  let list = await fileStorage.readFile(settingFile);
  ...
  fileStorage.writeFile(settingFile, list);
};
