...
function getEnvironment() {
  ...
  if (typeof uni !== 'undefined' && typeof uni.getSystemInfoSync === 'function') {
    try {
      ...
      if (systemInfo.uniPlatform === 'app') {
        console.log('✅ 确认为 App 环境')
        
        // App环境使用plus.io，不需要检查uni.getFileSystemManager
        if (typeof plus !== 'undefined' && plus.io) {
          console.log('✅ App环境下支持plus.io文件系统')
          return 'APP'
        } else {
          console.log('⚠️ App环境下plus.io不可用，降级处理')
          return 'APP_FALLBACK'
        }
      }
      ...
      if (systemInfo.uniPlatform === 'h5') {
        console.log('🌐 确认为 uni-app H5 环境')
        return 'H5'
      }
      
      // 微信小程序环境
      if (systemInfo.uniPlatform === 'mp-weixin') {
        console.log('📱 确认为微信小程序环境')
        return 'MP-WEIXIN'
      }
  ...
  if (process.env.UNI_PLATFORM) {
    console.log('🔧 通过 process.env 检测平台:', process.env.UNI_PLATFORM)
  ...
  if (typeof plus !== 'undefined') {
    console.log('📱 通过 plus 对象检测到 App 环境')
    return 'APP'
  }
  
  // 🎯 H5环境检测（放在后面，避免误判）
  if (typeof window !== 'undefined' && typeof document !== 'undefined') {
    console.log('🌐 检测到浏览器环境 (H5)')
    return 'H5'
  }
  ...
  console.log('❓ 无法确定运行环境，使用 UNKNOWN')
  return 'UNKNOWN'
}
...
export class FileSystemStorage {
  constructor() {
    try {
      ...
      if (this.environment === 'APP') {
        // App环境，使用plus.io模块
        if (typeof plus !== 'undefined' && plus.io) {
          console.log('✅ App环境检测到plus.io模块，使用原生文件系统')
          this.useLocalStorageFallback = false
          this.initPlusIO()
        } else {
          console.log('⚠️ App环境但plus.io不可用，使用localStorage降级')
        ...
        if (typeof uni.getFileSystemManager === 'function') {
          console.log('✅ 小程序环境使用uni.getFileSystemManager')
          this.useLocalStorageFallback = false
          this.initFileSystemManager()
        } else {
          console.log('⚠️ 小程序环境但getFileSystemManager不可用，使用localStorage降级')
          this.useLocalStorageFallback = true
          this.initLocalStorageFallback()
        }
      } else {
        // 其他环境，使用localStorage降级
        console.log('🌐 其他环境，使用localStorage降级')
        this.useLocalStorageFallback = true
        this.initLocalStorageFallback()
      }
  ...
  initGlobalConfig() {
    ...
    try {
      ...
      this.fileManager.requestFileSystem(plus.io.PUBLIC_DOCUMENTS, (fs) => {
        fs.root.getFile(this.configFile, {create: true}, (fileEntry) => {
          fileEntry.file((file) => {
            if (file.size === 0) {
              // 文件为空，创建默认配置
              fileEntry.createWriter((writer) => {
                writer.onwriteend = () => {
                  console.log('✅ 全局配置文件创建成功:', this.configFile)
                }
              ...
              console.log('✅ 全局配置文件已存在:', this.configFile)
            }
          }, (error) => {
  ...
  setupPlusIOCompatibility() {
    this.fs = {
        ...
        try {
          this.fileManager.requestFileSystem(plus.io.PUBLIC_DOCUMENTS, (fs) => {
            fs.root.getDirectory(dirPath, {create: true}, (dirEntry) => {
              ...
              // 忽略已存在的错误
              if (!error.code || error.code !== 10) { // 10 = 文件已存在
      ...
      readFileSync: (filePath, encoding) => {
        console.log(`📖 plus.io读取文件: ${filePath}`)
        try {
          this.fileManager.requestFileSystem(plus.io.PUBLIC_DOCUMENTS, (fs) => {
  ...
  initLocalStorageFallback() {
    ...
    try {
      console.log('正在初始化本地存储 fallback...')
      ...
      console.log('✅ 本地存储 fallback 初始化成功')
        ...
        this.fileManager.requestFileSystem(plus.io.PUBLIC_DOCUMENTS, (fs) => {
          fs.root.getFile(filePath, {}, (fileEntry) => {
            fileEntry.file((file) => {
              ...
              reader.onloadend = () => {
                try {
                  const content = reader.result ? JSON.parse(reader.result) : null
                  resolve(content)
                } catch (parseError) {
              ...
              if (filePath === this.configFile) {
                console.log(`📝 全局配置文件不存在，创建默认配置: ${filePath}`)
                this.createDefaultGlobalConfig().then(resolve).catch(() => resolve(null))
              } else {
            ...
            if (filePath === this.configFile) {
              console.log(`📝 全局配置文件获取失败，创建默认配置: ${filePath}`)
              this.createDefaultGlobalConfig().then(resolve).catch(() => resolve(null))
            } else {
              console.error(`获取文件失败: ${filePath}`, error)
              resolve(null)
    ...
    if (this.useLocalStorageFallback) {
      ...
      return {
        ...
        actualPaths: {
          ...
          userData: `${this.basePath}/data/cloud_users/{userId}`,
          userConfig: `${this.basePath}/data/cloud_users/{userId}/user.config.json`,
          works: `${this.basePath}/data/cloud_users/{userId}/works/{workId}`,
  ...
  logStoragePaths(userId = 'example_user') {
    ...
    console.log('📂 CWriter 存储路径信息:', paths)
    
    if (!this.useLocalStorageFallback) {
      console.log('🎯 实际路径示例:')
      console.log(`   配置文件: ${this.configFile}`)
      console.log(`   用户目录: ${this.basePath}/data/cloud_users/${userId}`)
      console.log(`   作品目录: ${this.basePath}/data/cloud_users/${userId}/works/`)
      console.log(`   日志文件: ${this.basePath}/logs/${userId}/operations.json`)
    }
  }
  ...
  getUserPath(userId) {
    if (this.useLocalStorageFallback) {
      return `mock_path/data/cloud_users/${userId}`  // H5 环境下的模拟路径
    }
    return `${this.basePath}/data/cloud_users/${userId}`
  }

  // 获取作品文件路径
  getWorkPath(userId, workId) {
    if (this.useLocalStorageFallback) {
      return `mock_path/data/cloud_users/${userId}/works/${workId}`  // H5 环境下的模拟路径
    }
  ...
  getUserConfigPath(userId) {
    if (this.useLocalStorageFallback) {
      return `mock_path/data/cloud_users/${userId}/user.config.json`  // H5 环境下的模拟路径
    }
  ...
  async createWork(userId, workData) {
    if (this.useLocalStorageFallback) {
      try {
        ...
        console.log(`✅ [Fallback] 作品创建成功: ${newWork.title}`)
        return newWork
      } catch (error) {
  ...
  // 更新作品信息
  updateWork(userId, workId, updates) {
    const workConfigPath = `${this.getWorkPath(userId, workId)}/work.config.json`
    ...
    return userConfig.works[workId]
  }
  ...
  getWorkDetail(userId, workId) {
    if (this.useLocalStorageFallback) {
      try {
        ...
        console.log(`✅ [Fallback] 获取作品详情成功: ${workDetail.title}`)
        return workDetail
      } catch (error) {
    ...
    // 加载作品内容文件
    const manuscript = this.readFile(`${workDir}/settings/manuscript.json`) || {}
  ...
  saveWorkContent(userId, workId, contentUpdates) {
    if (this.useLocalStorageFallback) {
      try {
        const result = this.saveWorkContentFallback(userId, workId, contentUpdates)
        console.log(`✅ [Fallback] 保存作品内容成功`)
        return result
      } catch (error) {
    ...
    try {
      if (contentUpdates.manuscript) {
        ...
        if (updatedManuscript.content) {
          updatedManuscript.word_count = updatedManuscript.content.split(/\s+/).length
        }
      ...
      if (contentUpdates.chapters) {
        this.writeFile(`${workDir}/chapters/chapters.json`, contentUpdates.chapters)
      }
      
      if (contentUpdates.glossary) {
        this.writeFile(`${workDir}/glossary/glossary.json`, contentUpdates.glossary)
      }
      
      if (contentUpdates.map_data) {
        this.writeFile(`${workDir}/maps/map_data.json`, contentUpdates.map_data)
      }
  ...
  addGlossaryItem(userId, workId, itemData) {
    const glossaryPath = `${this.getWorkPath(userId, workId)}/glossary/glossary.json`
  ...
  logOperation(userId, operation, data = {}) {
    try {
      ...
      let logs = this.readFile(logFile) || []
    ...
    try {
      ...
      if (!this.useLocalStorageFallback) {
        ...
        if (config && config.backups_count !== undefined) {
          config.backups_count[userId] = (config.backups_count[userId] || 0) + 1
          this.writeFile(this.configFile, config)
        }
      }
  ...
  getStorageStats(userId) {
    ...
    try {
      ...
      let totalSize = 0
      try {
        const config = !this.useLocalStorageFallback ? this.readFile(this.configFile) : this.getFallbackData()
        totalSize = JSON.stringify(config || {}).length + JSON.stringify(userConfig || {}).length
      } catch (error) {
        ...
        backupFiles.forEach(userId => {
          ...
          if (this.fileExists(userBackupDir)) {
            ...
            if (files.length > maxBackups) {
              // 删除最旧的备份文件
              files.sort().slice(0, files.length - maxBackups).forEach(file => {
  ...
  initUserStorageFallback(userId) {
    ...
    if (!data.data.cloud_users[userId]) {
    ...
    return data.data.cloud_users[userId]
  }
  
  // Fallback: 获取用户配置
  getUserConfigFallback(userId) {
    const data = this.getFallbackData()
    let userConfig = data.data.cloud_users[userId]
  ...
  createWorkFallback(userId, workData) {
    ...
    const newWork = {
      ...
      created_at: now,
      updated_at: now,
      local_file_path: `${this.basePath}/data/cloud_users/${userId}/works/${workId}`,
      content: {
        manuscript: {
          title: workData.title || '未命名作品',
          content: workData.initialContent || '',
  ...
  saveWorkContentFallback(userId, workId, contentUpdates) {
    const data = this.getFallbackData()
    const userConfig = data.data.cloud_users[userId]
    const work = userConfig.works[workId]
    ...
    if (contentUpdates.manuscript) {
      ...
      if (work.content.manuscript.content) {
        work.content.manuscript.word_count = work.content.manuscript.content.split(/\s+/).length
      }
    }
  ...
  getStorageStatsFallback(userId) {
    ...
    Object.values(userConfig.works || {}).forEach(work => {
      if (work.content && work.content.manuscript && work.content.manuscript.content) {
        totalWords += work.content.manuscript.content.split(/\s+/).length
        totalCharacters += work.content.manuscript.content.length
      }
      
      if (work.content && work.content.map_data && Array.isArray(work.content.map_data)) {
    ...
    return {
      ...
      totalCharacters,
      totalMaps,
      storageUsed: JSON.stringify(this.getFallbackData()).length,
      storageType: 'localStorage_fallback'
    }
  }
}
